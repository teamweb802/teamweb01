...
- 관광/교통/문화 소개: 현재 파일 기준 `sub11~sub53.html`, 설계 명칭 기준 `spot11~spot53.html`

### 1-2. `demo.zip`에서 확인된 백엔드 구조
Spring Boot 기반의 전형적인 MVC 구조다.

- Controller: 현재 구조 기준 `HomeController`, `SubController`, `MemberController`, `BoardController` / 적용 설계 기준 `SpotController` 사용
- Service: `MemberService`, `BoardService`
- Domain(기존 Entity): `Member`, `Board`
...
2. **공지사항 첨부파일 기능 불일치**
   ...
   - demo 백엔드 `Board` 도메인 모델에는 첨부파일 구조가 없음
...
현재 `sub11 ~ sub53` 계열(설계 명칭 `spot11 ~ spot53`)과 `privacy`, `copyright`, `terms`는 **템플릿 자원**으로 관리하는 것이 적절하다.
...
| 관광 정적 페이지 | `spotXX.html` | `/spot/spot11` 등 | `templates/spot/...` |
...
sub11.html ~ sub53.html   -> templates/spot/spot11.html ~ spot53.html
...
### 회원가입 화면
프론트에는 `email`, `tel`이 있으므로 백엔드 domain 모델과 DTO에 반드시 반영해야 한다.
...
4. `spotXX` 관광 페이지는 1차에서는 **정적 템플릿 유지**가 적절하다.
